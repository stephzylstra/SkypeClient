...
#define CONVERSATION_TIME 16200

@implementation Statistics

@synthesize conversationSessions;
@synthesize loggedIn;
...
- (id)init {
    ...
    if (self) {
        ...
        self.loggedIn =
        ((SkypeClientAppDelegate *)[[NSApplication sharedApplication]
                                    delegate]).loggedInUsername.stringValue;
...
- (NSArray *) sessionsForContact:(NSString *)contact: (NSString *)loggedInAccountName {
    ...
    NSArray *conversation = [[[Global _settings] fileProcessor]
                             getConversation:contact :loggedInAccountName];
    ...
    for (int i = 0; i < [[conversation objectAtIndex:0] count]; i++) {
        ...
        NSString *dateField = [[conversation objectAtIndex:0]
                               objectAtIndex:i];
        NSString *senderField = [[conversation objectAtIndex:1]
                                 objectAtIndex:i];
        NSString *messageField = [[conversation objectAtIndex:2]
                                  objectAtIndex:i];
        
        NSDate *currentDate = [df dateFromString:dateField];
        
        
        if (fabs([lastChecked timeIntervalSinceDate:currentDate]) > 
            CONVERSATION_TIME) {
        ...
        [currentSession addConversationLine:[[NSArray alloc] initWithObjects:
                                             dateField,
                                             senderField,
                                             messageField,
                                             nil]];
...
- (NSString *) mostFrequentChats:(NSString *) loggedInAccountName {
    ...
    [[((SkypeClientAppDelegate *)([[NSApplication sharedApplication] delegate]))
      selectContact] removeAllItems];
    
    NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:
                      [[NSString
                        stringWithFormat:@"~/Library/Application Support/SkypeClient/%@/",
                        loggedInAccountName]
                       stringByExpandingTildeInPath] error:NULL];
...
- (NSInteger) averageNumberOfLines:(NSString *) loggedInAccountName {
    ...
    for (int i = 0; i < [files count]; i++) {
        ...
        if ([[self conversationSessions]
             objectForKey:[files objectAtIndex:i]] == nil) {
            
            // current contact has not already had sessions calculated
            NSArray *contactSessions = [self sessionsForContact:
        ...
        for (int j = 0; j < [[[self conversationSessions] objectForKey:
                              [files objectAtIndex:i]] count]; j++) {
            
            
            totalLines += [[[[self conversationSessions] objectForKey:
                             [files objectAtIndex:i]] objectAtIndex:j]
                           numberOfLines];
            numSessions++;
        }
...
- (NSDictionary *) percentageOfChatsStarted: (NSString *) loggedInAccountName {
    ...
    for (int i = 0; i < [files count]; i++) {
        ...
        for (int j = 0; j < [[[self conversationSessions] objectForKey:
                              [files objectAtIndex:i]] count]; j++) {
            if ([[[[[self conversationSessions]
                    objectForKey:[files objectAtIndex:i]] objectAtIndex:j]
                  getSenderAtLine:0] isEqualToString:loggedInAccountName]) {
                started++;
            }
            sessions++;
        }
        ...
        [percentages setValue:[NSNumber numberWithDouble:percentage]
                       forKey:[files objectAtIndex:i]];
    }
...
- (NSInteger) averageNumberOfQuestions:(NSString *) loggedInAccountName {
    ...
    for (int i = 0; i < [files count]; i++) {
        ...
        for (int j = 0; j < [[[self conversationSessions]
                              objectForKey:[files objectAtIndex:i]]
                             count]; j++) {
            
            numSessions++;
            
            for (int k = 0; k < [[[[self conversationSessions]
                                   objectForKey:[files objectAtIndex:i]]
                                  objectAtIndex:j] numberOfLines]; k++) {
                
                NSRange range = [[[[[self conversationSessions] objectForKey:
                                    [files objectAtIndex:i]] objectAtIndex:j]
                                  getMessageAtLine:k] rangeOfString:@"?"];
...
- (NSInteger) averageResponseTime:(NSString *) loggedInAccountName {
    ...
    NSArray *files = [[NSFileManager defaultManager]
                      ...
                      [[NSString stringWithFormat:@"~/Library/Application Support/SkypeClient/%@/",
                        loggedInAccountName]
                       stringByExpandingTildeInPath] error:NULL];
    ...
    for (int i = 0; i < [files count]; i++) {
        ...
        if ([[self conversationSessions] objectForKey:[files objectAtIndex:i]] == nil) {
            // current contact has not already had sessions calculated
            NSArray *contactSessions = [self sessionsForContact:
                                        [files objectAtIndex:i]:loggedInAccountName];
        ...
        for (int j = 0; j < [[[self conversationSessions] objectForKey:
            ...
            for (int k = 1; k < [[[[self conversationSessions] objectForKey:
                                   [files objectAtIndex:i]] objectAtIndex:j]
                                 numberOfLines]; k++) {
...
- (NSInteger) averageLinesForContact:(NSString *)contact
                      whenLoggedInAs:(NSString *)loggedInAccountName {
    ...
    if ([[self conversationSessions] objectForKey:contact] == nil) {
        ...
        NSArray *contactSessions = [self sessionsForContact:contact:
                                    loggedInAccountName];
    ...
    for (int i = 0; i < [[[self conversationSessions]
                          objectForKey:contact] count]; i++) {
        totalLines += [[[[self conversationSessions] objectForKey:contact]
                        objectAtIndex:i] numberOfLines];
        numSessions++;
    }
...
- (NSDate *) lastMessageTimeForContact:(NSString *)contact
                        whenLoggedInAs:(NSString *)loggedInAccountName {
    ...
    return [[contactSessions objectAtIndex:[contactSessions count] - 1]
            getDateAtLine:[[contactSessions
                            objectAtIndex:[contactSessions count] - 1]
                           numberOfLines] - 1];
}

- (NSArray *) averageResponseTimeForContact:(NSString *)contact
                             whenLoggedInAs:(NSString *)loggedInAccountName {
    ...
    if ([[self conversationSessions] objectForKey:contact] == nil) {
        // current contact has not already had sessions calculated
        NSArray *contactSessions = 
        [self sessionsForContact:contact:loggedInAccountName];
    ...
    for (int j = 0; j < [[[self conversationSessions]
        ...
        for (int k = 1; k < [[[[self conversationSessions] objectForKey:contact]
                              objectAtIndex:j] numberOfLines]; k++) {
            ...
            if ([[[[[self conversationSessions] objectForKey:contact]
                   objectAtIndex:j] getSenderAtLine:k]
                 isEqualToString:[[[[self conversationSessions]
                                    objectForKey:contact] objectAtIndex:j]
                                  getSenderAtLine:lastCompared]]) {
                     // comparing against same person
                continue;
            }
            
            currentLineDate = [[[[self conversationSessions]
                                 objectForKey:contact] objectAtIndex:j]
                               getDateAtLine:k];
            previousLineDate = [[[[self conversationSessions]
                                  objectForKey:contact] objectAtIndex:j]
                                getDateAtLine:k-1];
                        
            double timeInterval = fabs([currentLineDate
                                        timeIntervalSinceDate:previousLineDate]);
            ...
            if (timeInterval == NAN) {
                NSLog(@"NaN with index %d of session %d", k, j);
                    ...
                    if ([[[[[self conversationSessions]
                            objectForKey:contact] objectAtIndex:j]
                          getSenderAtLine:k] isEqualToString:[[Global _settings]
                                                              loggedInAs]]) {
    ...
    NSNumber *averageYou = (totalResponseTimeYou != 0 && numLinesYou != 0) ?
    [NSNumber numberWithLongLong:((long long) totalResponseTimeYou / numLinesYou)]
    :[NSNumber numberWithInt:0];
    
    NSNumber *averageThem = (totalResponseTimeThem != 0 && numLinesThem != 0)
    ? [NSNumber numberWithLongLong:((long long) totalResponseTimeThem / numLinesThem)]:
    [NSNumber numberWithInt:0];
...
- (NSString *) stringFromTime:(double)time {
    ...
    // this shouldn't ever be reached as max session is 4.5 hours
