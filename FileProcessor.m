...
- (id)init {
    ...
    if (self) {
        ...
        self.rootPath = [@"~/Library/Application Support/SkypeClient/"
                         stringByExpandingTildeInPath];
        self.defaultImagePath = [@"~/Library/Application Support/SkypeClient/default.jpg"
         stringByExpandingTildeInPath];
    }
...
- (NSString *) imagePath: (NSString *)loggedInAccount {
    ...
    if (!([[NSFileManager defaultManager]
           fileExistsAtPath:rootPath isDirectory:&isDir])
        && isDir) {
        
        [[NSFileManager defaultManager]
         createDirectoryAtPath:rootPath withIntermediateDirectories:YES
         attributes:nil
         error:nil];        
    }
...
- (void) writeToConversation:(NSString *) conversation
    ...
    NSString *line = [[df stringFromDate:[NSDate date]]
                      stringByAppendingString:@": "];
    NSString *sndr = [[conversationLine objectAtIndex:0]
                      objectAtIndex:[[conversationLine objectAtIndex:0]
                                     count] -1];
    NSString *msg = [[conversationLine objectAtIndex:1]
                     objectAtIndex:[[conversationLine objectAtIndex:1]
                                    count] -1];
...
- (NSArray *) getConversation: (NSString *) conversation: (NSString *) loggedInAccount {
    ...
    for (NSInteger i = 0; i < [lines count]; i++) {
        ...
        NSArray *components = [[lines objectAtIndex:i]
        ...
        [[conversationMessages objectAtIndex:0]
         addObject:[components objectAtIndex:0]]; // date/time
        [[conversationMessages objectAtIndex:1]
         addObject:[components objectAtIndex:1]]; // sender
        [[conversationMessages objectAtIndex:2]
         addObject:[components objectAtIndex:2]]; // message
...
- (void) startAvatarCheck: (NSString *) contact {
    ...
    NSData *sending = [[[@"lv\n" stringByAppendingString:contact]
                        stringByAppendingString:@"\n"]
...
- (BOOL) hasAvatar: (NSString *)contact {
    return [[[[Global _settings] contactsAvatars] valueForKey:contact]
            boolValue];
}
