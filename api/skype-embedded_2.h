...
public:
        ...
        /** contact added or removed from this group */
...
public:
        ...
        enum PROPERTY {
                ...
                P_LAST_VOICE_ERROR = 947        /*!< [OTHERS], type: Sid::String*/,
                ...
                P_ADDER                         /*!< [ALL] who added this participant, type: Sid::String*/
        };
...
public:
        ...
        enum PROPERTY {
                ...
                P_CREATION_TIMESTAMP            /*!< timestamp of creation (doesn't apply to dialogs), type: uint*/,
        ...
        /** Setupkey SETUPKEY_CALL_NOANSWER_TIMEOUT type:int  @n Timeout in seconds after which the incoming live session will stop ringing (and if possible, proceed to voicemail or call forward). @n This is account-specific setup key. It can only be used while an account is logged in. @n  */
        ...
        /** Stops a voicemail recording and sends it (dialog only) */
        ...
        /** This callback gets fired when a message gets added or changed in the Conversation. @n 
        * @param message Message for which the OnMessage event got fired. @n 
        */
...
public:
        ...
        /** For Message types that include a body and are editable:  @li alters BODY_XML of the message object @li sets EDITED_BY and EDIT_TIMESTAMP properties  @li propagates the changes to remote users. @n  @n Specify isXML as true if the message body is formatted as XML; omit it or specify it as false if the message body is plain text. @n  */
        bool Edit(
        ...
        bool GetPropLeavereason(Message::LEAVEREASON& leavereason);
...
public:
        ...
        enum PROPERTY {
                ...
                P_DIMENSIONS                    /*!< 'width height', for example '640 480', type: Sid::String*/,
                P_MEDIA_TYPE                    /*!< 'width height', for example '640 480', type: MEDIATYPE*/,
        ...
        /** Assigns window handle to the video object, where the rendering should occur. @n  */
        bool SetScreen(
                const uint& windowh
        );
        
        /** This method starts either video send or video receive, depending on whether the video object is sender or receiver. If the video object is supposed to start rendering video locally, Video class Start method must be preceeded by calls to SetScreen and SetRenderRectangle @n  */
        bool Start(
        );
        
        /** This method stops either video send or video receive, depending on whether the video object is sender or receiver. @n  */
        bool Stop(
        );
        ...
        virtual void OnLastFrameCapture(
                const Sid::Binary& image,
                const uint& width,
                const uint& height
        );
 ...
 *  originator, and its BodyXml property contains the message length and title text in following format: @n  @n @code
...
public:
        ...
        enum PROPERTY {
                ...
                P_SUBJECT                       /*!< subject line, type: Sid::String*/,
 ...
 *  chain goes like this: @n @code  @n Conversation->Message->Transfer  @n @endcode  @n  @n The first part of the
 *  message (before the files section) comes from the Conversation::PostFiles body argument. For each file in the
 *  message, a file section is provided with three fields: @li file size in bytes @li index - more or less arbitrary
 *  order in which the transfers should be displayed in the UI @li file name. @n  @n For practical purposes, the
 *  Message::P_BODY_XML property is not all that useful in this context. The Transfer instances, however, contain the
 *  state and progress feedback for your UI. You retrieve these Transfer instances using Message::GetTransfers method.
 *  Since the sender can post multiple files to @n a Conversation using the same Message, Message:GetTransfers retrieves
 *  a list of Transfer instances - one per file per recipient. @n  @n You can determine the direction of particular
 *  Transfer instance by querying Transfer::P_TYPE (INCOMING/OUTGOING).  @n  @n You can implement a progress indicator
 *  by monitoring Transfer::P_BYTESTRANSFERRED. Note that when testing this on your local network, you will most likely
 *  catch these property change events at the beginning and the end of the transfer only - which does not look like too
 *  useful. However, for non-local network transfers where the transfer speeds are in the neighborhood of 200-300 KB per
 *  second, you should consider implementing progress feedback as being mandatory and expect to catch multiple property
 *  change events for all but the smallest files. @n  @n Another property of interest is Transfer::P_STATUS. The normal
...
public:
        ...
        enum PROPERTY {
                ...
                P_LOGOUTREASON = 73             /*!< Recognized values for the P_LOGOUTREASON property. @n , type: LOGOUTREASON*/,
                P_COMMITSTATUS = 78             /*!< Account::COMMITSTATUS, type: COMMITSTATUS*/,
                P_SUGGESTED_SKYPENAME = 72      /*!< suggested skypename. present if logoutreason==SKYPENAME_TAKEN, type: Sid::String*/,
...
public:
        ...
        */
        ...
        /** creates 'blank' contact search. add your own terms */
        ...
        /** new conversation item added or removed from the specified list */
        ...
        */
        ...
        /** This method returns a table in form of three string lists of equal length. The first list contains audio output device handles ('hw:0,0', 'hw:0,1', etc.) The second list contains descriptive names of those devices (Ensoniq AudioPCI etc.) The third list contains device product IDs. Note that the values in these lists depend on which audio engine you are running (SAL, PCM, RTP). @n  */
        ...
        /** create account if not found. ret false on error */
        bool GetAccount(
                const Sid::String& identity,
                AccountRef& account
        );
        ...
        /** Reads data from the specified stream. This method should be called from the OnApp2AppStreamListChange event callback, when this callback gets fired with listType argument set to RECEIVED_STREAMS. @n NB! There is currently no easy way to determine how many bytes were actually read from the stream. To get around this limitation, you can prefix all stream packets with a header that contains the packet payload size. @n  */
        bool App2AppRead(
                const Sid::String& appname, /*!< application ID @n */
                const Sid::String& stream, /*!< stream ID @n */
