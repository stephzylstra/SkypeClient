...
- (id)init {
    ...
    if(self) {
        ...
        self.popoverController = [[INPopoverController alloc]
                                  initWithContentViewController:viewController];
        self.popoverController.closesWhenPopoverResignsKey = YES;
        self.popoverController.color = [NSColor
                                        colorWithCalibratedWhite:1.0
                                        alpha:0.8];
...
- (int)numberOfRowsInTableView:(NSTableView *)tableView {
    ...
    return (int)[[(NSArray *)[[[Global _settings] conversationText]
                              objectForKey:[[Global _settings]
                                            currentConversation]]
                  objectAtIndex:0] count];
}
...
- (NSView *)tableView:(NSTableView *)tableView
   viewForTableColumn:(NSTableColumn *)tableColumn
    ...
    if ([[tableView identifier] isEqualToString:@"contactsPane"]) {
        ...
        result.textField.stringValue = [[[Global _settings] onlineContacts]
                                        objectAtIndex:row];        
        [result.imageView setImage:[[[Global _settings]
                                     fileProcessor]
                                    getAvatar:result.textField.stringValue]];
        
        if (row == [[Global _settings] selectedContact]) {
            [result setBackgroundStyle:NSBackgroundStyleDark];
            
            [[tableView rowViewAtRow:row makeIfNecessary:YES]
             setBackgroundColor:[NSColor
                                 colorWithCalibratedRed:0.55
                                 green:0.71
                                 blue:0.89
                                 alpha:1]];
        }
        ...
        if ([[tableColumn identifier] isEqualToString:@"statDescription"]) {
            result = [tableView makeViewWithIdentifier:@"statDescription"
                                                 owner:self];
            ...
            if ([[Global _settings] loggedInAs] != NULL && 
                [[Global _settings] loggedInAs] != nil) {
                
                switch (row) {
                    case 0:
                        if ([[[Global _settings] statistics]
                             mostFrequentChats:[[Global _settings] loggedInAs]]
                            != nil) {
                            
                            result.textField.stringValue = [[[Global _settings]
                                                             statistics]
                                                            mostFrequentChats:
                                                            [[Global _settings]
                                                             loggedInAs]];
                    ...
                    case 2:
                        if ([NSString stringWithFormat:@"%ld", 
                             [[[Global _settings] statistics] 
                              averageNumberOfLines:[[Global _settings] 
                                                    loggedInAs]]] != nil) {
                        result.textField.stringValue =
                    ...
                    case 4:
                        if ([[[Global _settings] statistics]
                             averageResponseTime:[[Global _settings]
                                                  loggedInAs]] != 0) {
                        result.textField.stringValue = 
                                 [NSString stringWithFormat:@"%ld",
                                  [[[Global _settings] statistics]
        ...
        NSArray *correctConversation = (NSArray *)[[[Global _settings]
                                                    conversationText]
                                                   objectForKey:
                                                   [[Global _settings]
                                                    currentConversation]];
        ...
        if ([[tableColumn identifier] isEqualToString:@"sender"]) {
            ...
            result.textField.stringValue = [[correctConversation
                                             objectAtIndex:0]
                                            objectAtIndex:row];
            [result.imageView setImage:[[[Global _settings]
                                         fileProcessor]
            ...
            NSRange range;
            NSString *textLine = [NSString stringWithString:
                                  [[correctConversation objectAtIndex:1]
                                   objectAtIndex:row]];
            
            [[[Global _settings] commandProcessor]
             getEmoticonRangeFromLine:textLine
             usingRange:&range];
            
            while (range.location != NSNotFound) {
                NSRange start = [[textLine substringWithRange:range]
                                 rangeOfString:@"\""];
                
                [test appendAttributedString:[[NSAttributedString alloc]
                                              initWithString:[textLine
                                                              substringToIndex:
                                                              range.location]]];
                
                NSRange end = [[[textLine substringWithRange:range]
                                substringFromIndex:start.location + 1]
                               rangeOfString:@"\""];
                
                NSRange emoticonNameRange = NSMakeRange(start.location + 1,
                                                        end.location);
                
                NSString *emoticonName = [[textLine substringWithRange:range]
                                          substringWithRange:emoticonNameRange];
                NSString *filePath = [[NSString
                                       stringWithFormat:@"~/Library/Application Support/SkypeClient/emoticons/%@_20.png", emoticonName]
                ...
                textLine = [textLine
                            ...
                            withString:@""];
                [[[Global _settings] commandProcessor]
                 getEmoticonRangeFromLine:textLine usingRange:&range];                
            }
            
            [test appendAttributedString:[[NSAttributedString alloc]
                                          initWithString:textLine]];
            
            if ([[[correctConversation objectAtIndex:1]
                  objectAtIndex:row] hasPrefix:@"FILE- "]) {
                ...
                if ([[[correctConversation objectAtIndex:0] objectAtIndex:row]
                    ...
                    filePath = [[@"~/Library/Application Support/SkypeClient/Files/"
                                 stringByExpandingTildeInPath]
                                stringByAppendingPathComponent:
                                [[fileObjects objectAtIndex:0]
                                 stringByReplacingOccurrencesOfString:@"\n"
                                 withString:@""]];
                }
                
                if ([[filePath pathExtension] isCaseInsensitiveLike:@"jpg"] || 
                    [[filePath pathExtension] isCaseInsensitiveLike:@"png"] || 
                    [[filePath pathExtension] isCaseInsensitiveLike:@"gif"]) {  
                    
                    [result.imageButton
                     setImage:[[NSImage alloc]
                               initWithContentsOfFile:filePath]];
                    [result.imageButton
                     setAlternateImage:[[NSImage alloc]
                                        initWithContentsOfFile:filePath]];
                    
                } else {
                    [result.imageButton setImage:[[[NSFileWrapper alloc]
                                                   initWithPath:filePath] icon]];
                    [result.imageButton setAlternateImage:
                     [[[NSFileWrapper alloc] initWithPath:filePath] icon]];
                }
...
- (IBAction)sendChatMessage:(id)sender {
    ...
    NSString *entered = [[[[[NSString alloc] init]
                           stringByAppendingString:@"hs\n"]
                          stringByAppendingString:[text stringValue]]
...
- (BOOL)tableView:(NSTableView *)pTableView
    ...
    for (NSString *filename in filenames) {
        [NSThread detachNewThreadSelector:@selector(sendFileTransfer:)
                                 toTarget:[[NSApplication sharedApplication]
                                           delegate] withObject:filename];
    }
...
- (IBAction)loadPopover:(id)sender {
    ...
    if (self.popoverController.popoverIsVisible) {
        ...
        [self.popoverController
         showPopoverAtPoint:NSMakePoint(NSMidX(buttonBounds),
                                        NSMidY(buttonBounds))
         inView:sender
