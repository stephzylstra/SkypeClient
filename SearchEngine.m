...
- (void) performSearch:(NSString *)searchTerm forUser:(NSString *)loggedInUser {
    ...
    NSString *filePath = [[@"~/Library/Application Support/SkypeClient/"
                           stringByAppendingString:loggedInUser]
                          stringByExpandingTildeInPath];
    
    NSArray *files = [[NSFileManager defaultManager]
                      contentsOfDirectoryAtPath:filePath error:NULL];
    ...
    for (int i = 0; i < [files count]; i++) {
        ...
        NSArray *conv = [[[Global _settings] fileProcessor]
                         getConversation:[files objectAtIndex:i]:loggedInUser];
        
       for (int j = 0; j < [[conv objectAtIndex:0] count]; j++) {
            NSRange range = [[[conv objectAtIndex:2] objectAtIndex:j]
                             rangeOfString:searchTerm];
            if (range.location != NSNotFound) {
                
                [[[self searchResults] objectAtIndex:0]
                 addObject:[[conv objectAtIndex:1] objectAtIndex:j]];
                [[[self searchResults] objectAtIndex:1]
                 addObject:[[conv objectAtIndex:2] objectAtIndex:j]];
